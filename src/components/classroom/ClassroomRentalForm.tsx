...
const inputCls = 'w-full h-10 px-3.5 rounded-xl border border-base text-sm bg-surface-raised text-base-primary placeholder:text-base-muted/50 focus:outline-none focus:border-brand-indigo transition-colors'
  ...
  if (requestNumber) {
    return (
      <div className="bg-surface-base rounded-2xl border border-base p-8 text-center space-y-5">
        <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-indigo-50 dark:bg-brand-indigo-muted border border-indigo-200 dark:border-brand-indigo-border/30">
        ...
        <div className="bg-surface-raised rounded-xl border border-base p-4">
          <p className="text-[11px] font-bold text-base-muted uppercase tracking-wider mb-2">대여 신청 번호</p>
          <p className="font-mono text-lg font-black text-brand-indigo">{requestNumber}</p>
        </div>
        ...
        <div className="flex gap-2 justify-center">
          <a href="/classrooms" className="h-10 px-5 rounded-xl border border-base text-sm font-semibold text-base-secondary hover:bg-surface-overlay hover:text-base-primary transition-colors flex items-center">목록으로</a>
          <a href="/status" className="h-10 px-5 rounded-xl bg-indigo-600 dark:bg-brand-indigo hover:bg-indigo-700 dark:hover:bg-indigo-400 hover:opacity-95 text-white dark:text-zinc-950 text-sm font-bold transition-colors flex items-center gap-1.5">
  ...
  return (
    <div className="bg-surface-base rounded-2xl border border-base p-6 space-y-5">
      <div className="flex items-center gap-3 bg-surface-raised rounded-xl px-4 py-3 border border-base">
        ...
        <span className="text-sm font-semibold text-base-primary">{classroomName} 대여 신청</span>
      </div>
      ...
      <form action={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div className="space-y-1.5">
            <label htmlFor="password" className="block text-xs font-medium text-base-secondary">비밀번호 (신청조회용) *</label>
            <input
              id="password"
              name="password"
              type="password"
        ...
        <div className="space-y-1.5">
          <label htmlFor="purpose" className="block text-xs font-medium text-base-secondary">대여 목적</label>
          <textarea
            id="purpose"
            name="purpose"
            className="w-full min-h-[70px] max-h-[140px] px-3.5 py-2.5 rounded-xl border border-base text-sm bg-surface-raised text-base-primary placeholder:text-base-muted/50 focus:outline-none focus:border-brand-indigo transition-colors"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
          <div className="space-y-1.5">
            <label className="block text-xs font-medium text-base-secondary">대여 시작 날짜 시간 *</label>
            <DateTimePicker value={startAt} onChange={setStartAt} />
          </div>
          <div className="space-y-1.5">
            <label className="block text-xs font-medium text-base-secondary">반납 예정 날짜 시간 *</label>
        ...
        {needsApproval && (
          <div className="space-y-3 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/40 rounded-xl p-4 animate-in fade-in slide-in-from-top-1 duration-200">
            ...
            <label className="flex items-center gap-2.5 p-2 rounded-lg bg-surface-raised border border-base hover:border-strong cursor-pointer transition select-none">
              <input
                ...
                className="w-4.5 h-4.5 rounded-md accent-indigo-600 dark:accent-brand-indigo border-base focus:ring-transparent"
              />
        ...
        {error && (
          <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/50 rounded-xl p-3 text-center text-xs font-bold text-red-600 dark:text-red-400">
            {error}
          </div>
        )}
        ...
        <button
          ...
          className="w-full h-11 inline-flex items-center justify-center gap-1.5 rounded-xl bg-indigo-600 dark:bg-brand-indigo text-white dark:text-zinc-950 hover:bg-indigo-700 dark:hover:bg-indigo-400 hover:opacity-95 disabled:bg-surface-base disabled:border disabled:border-base disabled:text-base-muted/50 transition duration-200"
        >
